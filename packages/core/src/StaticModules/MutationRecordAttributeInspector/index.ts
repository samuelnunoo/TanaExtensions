...
export default new class MutationRecordAttributeInspector extends TanaConstants {
    ...
    public  getMutationEventType(mutation:MutationRecord) {
    ...
    public  nodeHasClassPrefix(node:HTMLElement,classPrefix:string) {
    ...
    public mutationRecordContainsBulletModule(mutation:MutationRecord) {
        const isRemoval = mutation.removedNodes.length > 0
    ...
    public  mutationRecordTargetHasExpectedTagName(target:HTMLElement) {
        if (!target) return false
        return (target as HTMLElement).tagName == "DIV" || (target as HTMLElement).tagName == "SPAN"
    }

    public  mutationRecordTargetHasExpectedClass({target}:MutationRecord) {
